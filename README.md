...
#### Common工具类 - 基础设施
- **`RedisService.java`** - Redis操作工具类
  - 封装常用Redis操作(set/get/del/expire)
  - 支持原子操作和批量操作
  - 提供分布式锁基础功能
...
#### 消息队列组件
- **`MessageSender.java`** - 消息发送工具
  - 支持延迟消息、死信队列处理
  - 消息重试机制和失败回调
  
- **`PaymentProcessListener.java`** - 支付消息监听器
  - 异步处理支付成功/失败事件
  - 订单状态更新和库存同步
